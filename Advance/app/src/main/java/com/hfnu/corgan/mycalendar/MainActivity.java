...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == getResources().getInteger(R.integer.login_ok)){
            ...
            addCalendar(date, title, beginTime, endTime, "", position.substring(position.indexOf("(") + 1, position.length() - 1));
        }
    }

    void addCalendar(String date, String title, String start, String end, String comment, String position){
        Resources resources = getResources();
        String urlStr = resources.getString(R.string.server_url) + "?addCal=&user=" + users.getUsername() + "&date=" + date + "&title=" + title
                + "&start=" + start + "&end=" + end + "&comment=" + comment + "&position=" + position;
        jsonThread.pushUrlStr(urlStr);
    }

    void onAddCalOk(){
        Toast.makeText(this, R.string.message_add_calendar_ok, Toast.LENGTH_SHORT).show();
    }
